...
public class TemplateManager 
{
	...
	private void initTemplates()
	{
		...
		for (int i = 0; i < nodeList.getLength(); i++)
		{
			...
			template.setChart(ChartManager.getDefault().getChart(XMLUtil.getNameAttr(chart)));
			...
			for (int j = 0; j < overlaysList.getLength(); j++)
			{
				...
				Overlay overlay = OverlayManager.getDefault().getOverlay(
			...
			for (int j = 0; j < indicatorsList.getLength(); j++)
			{
				...
				Indicator indicator = IndicatorManager.getDefault().getIndicator(
	...
	public void saveToTemplate(String name, ChartFrame chartFrame)
	{
		...
		List<Overlay> overlays = chartFrame.getMainPanel().getSplitPanel()
			.getChartPanel().getOverlays();
		if (!overlays.isEmpty())
		{
		...
		List<Indicator> indicators = chartFrame.getMainPanel().getSplitPanel()
			.getIndicatorsPanel().getIndicatorsList();
	...
	public void removeTemplate(String name)
	{
		...
		for (int i = 0; i < nodeList.getLength(); i++)
		{
			...
			if (node.getAttributes().getNamedItem(XMLUtil.NAME_ATTR)
